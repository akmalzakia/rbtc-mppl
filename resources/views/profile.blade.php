    ...
    <div class="w-2/3 flex flex-col space-y-1">
      ...
      <x-partial.cards.card class="rounded-md h-screen flex-1">
        <ul class="flex border-b border-gray-100">
          <li class="flex-1 border-b {{ (request('type') === 'magazine') || (!request('type')) ? 'border-blue-600' : 'border-transparent group hover:border-blue-400 transition duration-300'}}">
            <a class="relative block p-4" href="?type=magazine">
              ...
              <div class="flex items-center justify-center">
                  <button type="submit" class="ml-3 text-sm font-medium {{ (request('type') === 'magazine') || (!request('type')) ? 'text-blue-600' : 'text-gray-900 group-hover:text-blue-400 transition duration-300' }}"> Majalah </button>
              </div>
            </a>
          </li>

          <li class="flex-1  border-b {{ (request('type') === 'textbook') ? 'border-blue-600' : 'border-transparent group hover:border-blue-400 transition duration-300'}}">
            <a class="relative block p-4" href="?type=textbook">
              <div class="flex items-center justify-center">
                <span class="ml-3 text-sm font-medium {{ (request('type') === 'textbook') ? 'text-blue-600' : 'text-gray-900 group-hover:text-blue-400 transition duration-300' }}">
                  Textbook </span>
              </div>
            </a>
          </li>

          <li class="flex-1 border-b {{ (request('type') === 'paper') ? 'border-blue-600' : 'border-transparent group hover:border-blue-400 transition duration-300'}}">
            <a class="relative block p-4" href="?type=paper">
              <div class="flex items-center justify-center">
                <span class="ml-3 text-sm font-medium {{ (request('type') === 'paper') ? 'text-blue-600' : 'text-gray-900 group-hover:text-blue-400 transition duration-300' }}">
                  Paper </span>
              </div>
            </a>
          </li>
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="flex w-full p-2 border-b shadow-sm">
            ...
            <div class="flex flex-col justify-between px-2 pt-2 w-3/4">
              <div>
                <p class="font-bold text-xl text-blue-600">{{ $bookmark->book->name }}</p>
              ...
              <div>
                ...
                <div class="flex justify-between border-t pt-1">
                  ...
                  <a href="/bookmark/{{ $bookmark->id }}" class="text-blue-600 text-lg">Continue Reading >></a>
                </div>
              </div>
            </div>
          </div>
